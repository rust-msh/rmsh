...
def export_all_dimensions_to_step(out_path: Path) -> None:
    ...
    try:
        ...
        # Strict gmsh-compatible STEP profile with explicit neutral body color
        # so viewers do not render white-on-white by default.
        rmsh.option.setNumber("STEP.GmshStrict", 1)
        rmsh.option.setString("STEP.Protocol", "AP214")
        rmsh.option.setColor("STEP.SolidColor", 180, 180, 180, 255)
...
if __name__ == "__main__":
    output = Path(__file__).resolve().parent / "out_all_dimensions_step_viewer_safe" / "all_dimensions_demo_viewer_safe.step"
