...
def export_all_dimensions_to_step_rmsh_viewer_ok(out_path: Path) -> None:
    rmsh.initialize()
    try:
        rmsh.clear()
        rmsh.model.add("all_dimensions_demo_rmsh_viewer_ok")

        # Keep strict disabled for this viewer-oriented sample.
        # The strict periodic seam path can trigger viewer/mesher issues
        # on mixed-dimension periodic solids in current rmsh builds.
        ...
        # 3D solids (avoid torus in this viewer-safe sample)
        ...
        # gmsh-compatible cone signature:
...
if __name__ == "__main__":
    output = Path(__file__).resolve().parent / "out_all_dimensions_step" / "all_dimensions_demo_rmsh_viewer_ok.step"
    export_all_dimensions_to_step_rmsh_viewer_ok(output)
