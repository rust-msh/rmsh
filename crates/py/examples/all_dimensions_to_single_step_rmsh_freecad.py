...
def export_all_dimensions_to_step_rmsh_freecad(out_path: Path) -> None:
    rmsh.initialize()
    try:
        rmsh.clear()
        rmsh.model.add("all_dimensions_demo_rmsh_freecad")

        # Keep default STEP writer behavior for better FreeCAD compatibility.
        # (Do not enable STEP.GmshStrict here.)
...
if __name__ == "__main__":
    output = Path(__file__).resolve().parent / "out_all_dimensions_step" / "all_dimensions_demo_rmsh_freecad.step"
    export_all_dimensions_to_step_rmsh_freecad(output)
